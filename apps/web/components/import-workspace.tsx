...
type MappingField=Field|'platformFee';
const requiredFields:Field[]=['orderId','sku','quantity','date','netSales'];
const mappingFields:MappingField[]=[...requiredFields,'platformFee'];
const labels:Record<MappingField,string>={orderId:'เลขคำสั่งซื้อ',sku:'รหัส SKU',quantity:'จำนวน',date:'วันที่ขาย',netSales:'เงินรับสุทธิต่อรายการ (บาท)',platformFee:'ค่าธรรมเนียมแพลตฟอร์ม (บาท) · ไม่บังคับ'};
const emptyMapping:Mapping={orderId:'',sku:'',quantity:'',date:'',netSales:'',platformFee:''};
const suggestedHeaders:Record<MappingField,string[]>={orderId:['order_id','เลขคำสั่งซื้อ'],sku:['sku','รหัส sku'],quantity:['quantity','จำนวน'],date:['date','วันที่ขาย'],netSales:['net_receipt','net_sales','เงินรับสุทธิ'],platformFee:['platform_fee','ค่าธรรมเนียมแพลตฟอร์ม']};
...
export function ImportWorkspace({tenantId,shopId,shopName,canWrite,drafts,initialPreview,initialDraftId,initialCommitted=false}:{tenantId:string;shopId:string;shopName:string;canWrite:boolean;drafts:DraftRow[];initialPreview?:Preview;initialDraftId?:string;initialCommitted?:boolean}){
  ...
  async function validate(){
    ...
    try{setPreview(await request<Preview>(endpoint,{action:'preview',input:{...source,mapping:{...mapping,platformFee:mapping.platformFee??''}} satisfies ImportInput}));setStep(3);}catch(e){setError(e instanceof Error?e.message:'ตรวจไฟล์ไม่สำเร็จ');}finally{setBusy(false);}
  }
  async function save(){
    if(!source)return;setBusy(true);setError('');
    try{const result=await request<{id:string;duplicate:boolean}>(endpoint,{action:'save',input:{...source,mapping}});setDraftId(result.id);setSaved(result.duplicate?'ไฟล์และการจับคู่คอลัมน์นี้มีร่างที่บันทึกไว้แล้ว':'บันทึกร่างแล้ว ยังไม่รวมเป็นยอดขาย ตรวจความหมายของคอลัมน์ก่อนยืนยันนำเข้า');router.refresh();}catch(e){setError(e instanceof Error?e.message:'บันทึกร่างไม่สำเร็จ');}finally{setBusy(false);}
  }
  ...
  return <>
    ...
    {step===2&&inspection&&source&&<section className="panel"><div className="panel-heading"><div><h2>จับคู่คอลัมน์</h2><p className="field-help">{source.filename} · {inspection.total} รายการ</p></div></div><p className="mapping-note">“เงินรับสุทธิ” ต้องเป็นยอดของรายการหลังหัก/บวกการปรับปรุงจากแพลตฟอร์มที่รวมอยู่ในตัวเลขนี้แล้ว ไม่ใช่ราคาต่อชิ้นหรือยอดโอนรวมทั้งร้าน ส่วนค่าธรรมเนียมเป็นข้อมูลประกอบและจะไม่ถูกหักซ้ำ</p><div className="mapping-list">{mappingFields.map(f=><div className="mapping-row" key={f}><label>{labels[f]}<select aria-label={`คอลัมน์ ${labels[f]}`} value={mapping[f]??''} onChange={e=>setMapping({...mapping,[f]:e.target.value})} disabled={busy}><option value="">{f==='platformFee'?'ไม่ใช้คอลัมน์นี้':'เลือกคอลัมน์ในไฟล์'}</option>{inspection.headers.map(h=><option key={h}>{h}</option>)}</select></label><div className="mapping-sample"><span>ตัวอย่างจากไฟล์</span><code>{mapping[f]?inspection.sample.map(r=>r[inspection.headers.indexOf(mapping[f]!) ]).join(' · '):'—'}</code>{f==='date'&&<small>รูปแบบ YYYY-MM-DD เช่น 2026-09-16</small>}</div></div>)}</div><div className="form-actions"><button type="button" className="button primary" disabled={busy||requiredFields.some(f=>!mapping[f])} onClick={validate}>{busy?'กำลังตรวจ…':'ตรวจรายการ'}</button></div></section>}
    {step===3&&preview&&<>
