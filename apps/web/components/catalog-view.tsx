...
export function CatalogView({items,canReadCosts,canWrite,limit,saved,focused}:{items:CatalogItem[];tenantId:string;shopId:string;canReadCosts:boolean;canWrite:boolean;limit:number;saved?:string;focused?:string}){
  const [query,setQuery]=useState(''),[category,setCategory]=useState('all'),[missingOnly,setMissingOnly]=useState(false);
  ...
  const filtered=groups.filter(g=>(category==='all'||g[0].category===category)&&(!missingOnly||g.some(i=>i.costMinor==null))&&g.some(p=>`${p.name} ${p.sku} ${Object.values(p.attributes).join(' ')}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())));
  ...
  return <>
    <div className="page-heading"><div><h1>สินค้า</h1><p>{groups.length} สินค้า · {items.length} SKU ในร้านที่เลือก{items.length===limit?` · แสดงสูงสุด ${limit} SKU`:''}</p></div>{canWrite&&<Link href="/catalog/new" className="button primary"><Plus size={17}/>เพิ่มสินค้า</Link>}</div>
    ...
    <section className="panel table-panel"><div className="table-toolbar"><label className="search-field"><Search size={17}/><input aria-label="ค้นหาสินค้า" placeholder="ค้นหาชื่อสินค้า หรือ SKU" value={query} onChange={e=>setQuery(e.target.value)}/></label><select aria-label="กรองหมวดสินค้า" value={category} onChange={e=>setCategory(e.target.value)}><option value="all">ทุกหมวดสินค้า</option>{categories.map(c=><option key={c}>{c}</option>)}</select></div>
      ...
      <div className="table-footer"><span>{filtered.length} สินค้า{items.length===limit?' · รายการนี้แสดงข้อมูลเพียง 200 SKU แรก':''}</span><span>ราคาเป็นบาทต่อหน่วยขาย</span></div>
    </section>
  </>;
}
