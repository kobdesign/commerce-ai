import { settlements } from '@commerce/finance';
...
export default async function SettlementsPage(){
  ...
  const ledger=await settlements(w.ctx,w.shop.id);
  const today=new Date(Date.now()+7*60*60*1000).toISOString().slice(0,10);
  return <SettlementWorkspace key={`${w.org.id}:${w.shop.id}`} tenantId={w.org.id} shopId={w.shop.id} shopName={w.shop.name} canWrite={w.ctx.role!=='auditor'} today={today} ledger={ledger}/>;
}
