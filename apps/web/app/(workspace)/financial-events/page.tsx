...
export default async function FinancialEventsPage(){
  ...
  return <FinancialEventWorkspace key={`${w.org.id}:${w.shop.id}:${ledger.asOf}`} tenantId={w.org.id} shopId={w.shop.id} shopName={w.shop.name} canWrite={w.ctx.role!=='auditor'} today={today} ledger={ledger}/>;
}
