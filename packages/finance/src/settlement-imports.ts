...
export type SettlementImportBatch={id:string;filename:string;byteSize:number;sourceHash:string;schemaVersion:string;total:number;payoutCount:number;unmatchedLineCount:number;committedAt:string};
...
export async function settlementImportBatches(ctx:Context,shopId:string):Promise<SettlementImportBatch[]>{
  return withTenant(ctx,async(tx,role)=>{
    ...
    const rows=await tx.query<Omit<SettlementImportBatch,'committedAt'>&{committedAt:Date}>(`SELECT id,filename,byte_size AS "byteSize",source_hash AS "sourceHash",schema_version AS "schemaVersion",
